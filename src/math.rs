...
pub fn probabilities(occurences: &[u64]) -> Vec<f64> {
...
pub fn simpsons_d(probabilities: &[f64]) -> f64 {
    1. - probabilities.iter().fold(0., |sum, val| sum + val * val)
}

pub fn simpsons_d_of_one(probabilities: &[f64]) -> f64 {
...
/// asc.
pub fn letis_d(ranked_probabilities: &[f64]) -> f64 {
    let mut total = 0.;
...
pub fn letis_d_of_one(ranked_probabilities: &[f64]) -> f64 {
